...
1. 빌드 과정
   빌드 > 빌드된 output 파일을 index.html에 추가 > 라이브 서버에서 구동하기

2. webpack dev server
   빌드 없이, 명령어를 바탕으로 실행 > 웹팩에 설정해둔 경로를 바탕으로 static output을 만드는 것이 아닌, 메모리에 저장하고 바로 출력

<img src="./images/devserver1.png">

하지만 해당 열린 포트에 대해서 직접 접근해서 확인해보면 bundle.js가 들어있는 것을 확인할 수 있다.

<img src="./images/devserver2.png">

이는 우리가 dev-server를 사용하기 위해 webpack에 추가적인 설정을 해줬기 때문이다.

웹팩 데브 서버를 사용하여 따로 번들링된 output을 파일 시스템으로 만들지 않고 메모리 내에서만 볼 수 있도록 만들어 준 것이다.

이에 대해서는 실습을 통해 자세히 알아본다.
...
module.exports = {
  mode: "none",
  // index.js를 바탕으로 번들링한다
  entry: "./index.js",
  // 루트디렉토리에 dist라는 폴더를 만들고, bundle.js라는 파일로 번들링한 결과물을 나타낸다
  output: {
    filename: "bundle.js",
    path: path.resolve(__dirname, "dist"),
  },
  ...
  // 플러그인으로 HtmlWebpackPlugin을 사용한다
  plugins: [
